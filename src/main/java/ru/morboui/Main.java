...
//    String nowAsISO = df.format(new Date());
        ...
        DateTime currentTime = new DateTime().toDateTime(DateTimeZone.forID("Europe/Moscow"));
            ...
            element.forEach(it -> {
                ...
                HttpRequest request = HttpRequest.newBuilder()
                        .uri(URI.create(
                                "https://api.rasp.yandex.net/v3.0/search/?apikey=" + System.getenv("YANDEX_API_KEY")
                                        + "&format=json&from=s9739796&to=s9744794&page=1&date=" + currentTime
                                        + "&levent=departure"))
                        .build();
                ...
                try {
                    ...
                    ya.segments.forEach(bus -> {
                        ...
                        DateTime arrTime = DateTime.parse(bus.arrival);
                        TemporalAccessor ta = DateTimeFormatter.ISO_INSTANT.parse(s);
                        Instant i = Instant.from(ta);
                        Date d = Date.from(i);

                        if (currentTime.isBefore(arrTime))
                            bot.execute(new SendMessage(it.message().chat().id(), "Next bus in: " + d));
